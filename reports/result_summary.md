...
| E04 | 对抗训练防御 | 已完成 | `results/03_defense/adversarial_training/` |
| E05 | Grad-CAM 可解释性 | 已完成 | `results/04_explainability/gradcam/` |
| E06 | JPEG quality 消融 | 已完成 | `results/03_defense/jpeg_quality_ablation/` |
| E07 | 类别鲁棒性与失败案例 | 已完成 | `results/04_extended_analysis/per_class_failure_analysis/` |
| E08 | JPEG 自适应 BPDA 攻击 | 已完成 | `results/04_extended_analysis/adaptive_jpeg_bpda/` |
| E09 | 运行效率统计 | 已完成 | `results/06_runtime/` |
...
5. 输入预处理防御仍存在上限，失败案例和 BPDA 自适应攻击说明不能把 JPEG 当作完备安全方案。
6. FGSM 对抗训练作为模型级防御显著提升鲁棒性，PGD epsilon=0.03 准确率提升到 0.8863。
7. 后续可继续补充 PGD/TRADES 训练、跨模型结构消融和交互式 Demo。
