...
def add_cover(doc):
    ...
    rows = [
        ...
        ("核心算法", "ResNet18 小尺寸图像适配版 + FGSM/PGD 对抗攻击 + 输入预处理防御 + FGSM 对抗训练"),
...
def part1(doc, data):
    ...
    add_para(
        doc,
        "因此，本报告的主线不是单纯复现一个 ResNet 分类模型，而是把交通标志识别放到安全评估语境中：第一步证明模型确实学会了正常识别，第二步用数学上受约束的扰动攻击检验模型局部决策边界，第三步用输入预处理和对抗训练两类防御方法观察鲁棒性能否恢复。这样安排的原因是，如果只展示 clean accuracy，报告只能说明模型会分类；只有继续展示攻击、对照、防御和失败边界，才能说明我们真正理解了模型为什么会错、怎样会被攻击、以及防御方案能解决到什么程度。",
    )
    ...
    rows = [
        ...
        ("防御实验", "Gaussian Blur、Median Filter、JPEG Compression、FGSM adversarial training", "同时比较输入级防御与模型级防御"),
        ("补充验证", "随机噪声对照、PGD 步数消融、防御参数扫描、自适应攻击、类别鲁棒性", "证明攻击不是普通噪声，并分析防御边界"),
    ]
    table(
        doc,
        "任务定义",
    ...
    add_para(
        doc,
        "这条流程也是后续证据链的组织方式：基础识别结果负责回答“模型是否有足够强的正常能力”；攻击实验回答“小扰动能否破坏这个能力”；扰动可感知性分析回答“攻击是否只是明显改图”；随机噪声和 PGD 步数消融回答“破坏是否来自梯度优化”；输入防御、参数扫描和 JPEG 消融回答“是否存在可部署的轻量恢复方法”；失败案例、BPDA 自适应攻击和对抗训练则回答“防御边界在哪里，以及模型级防御是否更有效”。",
    )
...
def part2(doc, data):
    ...
    add_para(
        doc,
        "从识别链路看，一张交通标志图像并不是直接被模型“看懂”的，而是先被转成数字张量。卷积层在局部窗口中提取边缘、颜色、数字笔画和几何形状，深层特征逐渐把局部纹理组合成类别相关的高层表示，最后由全连接分类层输出 43 个类别分数。训练时，交叉熵损失要求真实类别的 logit 相对其他类别更高；反向传播会把这个误差信号传回每一层卷积核，使网络逐步形成适合交通标志的特征提取器。",
    )
    add_para(
        doc,
        "这个机制也解释了为什么后续可以做对抗攻击：只要模型是可微的，损失函数不仅能对参数求梯度，也能对输入图像求梯度。训练阶段用参数梯度来让模型更正确；攻击阶段则反过来用输入梯度寻找会让损失增大的像素方向。也就是说，对抗攻击不是随机涂改图片，而是利用深度网络自身的可微结构来寻找敏感方向。",
    )
    ...
    add_para(
        doc,
        "选择 ResNet18 的原因有三点。第一，它足够经典，残差结构清晰，便于在报告中解释模型结构和数学形式；第二，它比浅层 CNN 更有表达能力，能够学习交通标志中的颜色、边框、数字和方向符号等多层次特征；第三，它的规模适中，既能训练出较高 clean accuracy，又能在有限时间内完成 FGSM/PGD 攻击、防御消融、Grad-CAM 和对抗训练等大量补充实验。对于课程展示而言，ResNet18 既有技术深度，也不会因为模型过大而拖慢完整实验链路。",
    )
    ...
    add_para(
        doc,
        "这两种攻击放在一起，能形成强弱有层次的攻击证据。FGSM 回答“只用一次梯度，模型是否已经脆弱”；PGD 回答“如果攻击者在同一扰动预算内反复优化，模型会不会更容易失败”。因此后文不会只报告一个攻击准确率，而会结合 epsilon 扫描、FGSM/PGD 对比、PGD 步数消融和随机噪声对照，逐步说明攻击效果来自梯度优化，而不是来自肉眼明显的图像破坏。",
    )
    ...
    h2(doc, "2.4.2 模型级防御：FGSM 对抗训练原理")
    add_para(
        doc,
        "输入预处理防御只改变测试时输入，而对抗训练会改变模型参数。其核心思想是在训练阶段主动生成对抗样本，把 clean loss 和 adversarial loss 一起反向传播，使模型在原图附近的 epsilon 邻域内学习更稳定的分类边界。本项目采用 FGSM 对抗训练微调：从已训练好的 ResNet18 best checkpoint 出发，每个 batch 先生成 FGSM 样本，再用 clean 与 adversarial 两部分损失联合更新参数。",
    )
    add_formula(doc, "min_theta  E_{(x,y)} [ alpha * L(f_theta(x), y) + beta * L(f_theta(x_adv), y) ]")
    add_formula(doc, "x_adv = Clip(x + epsilon * sign(grad_x L(f_theta(x), y)))")
    rows = [
        ("输入预处理", "测试前处理图像，模型参数不变", "部署简单、成本低", "面对自适应攻击可能被绕过"),
        ("对抗训练", "训练时混入对抗样本并更新模型参数", "直接调整决策边界，鲁棒性提升更明显", "需要训练成本，也可能影响 clean accuracy"),
    ]
    table(
        doc,
        "输入级防御与模型级防御对比",
        ...
        rows,
        [1500, 3100, 2500, 2260],
        "本表解释为什么需要补充对抗训练。输入预处理证明了简单部署方案的有效性；对抗训练则进一步回答“能否从模型本身提升鲁棒性”。两者构成更完整的防御链路。",
        font_size=8.2,
    )
...
def part3(doc, data):
    ...
    add_para(
        doc,
        "数据与预处理决定了后续实验是否可解释。对抗鲁棒性实验尤其需要保持数据链路稳定：如果训练、攻击和防御阶段使用不同的输入尺寸、归一化参数或随机增强方式，那么攻击效果和防御效果就难以直接比较。因此本项目先固定 GTSRB 数据集、输入分辨率、Normalize 参数和评估索引，再在同一批测试样本上依次执行 clean、attack、defense 和补充验证。",
    )
    add_para(
        doc,
        "GTSRB 的特点是类别多、样本数量不均衡、图像尺寸不统一，并且存在光照、模糊、角度和背景差异。对基础识别来说，这要求模型具备较好的泛化能力；对攻击实验来说，这些真实拍摄差异也让报告更接近实际交通场景，而不是只在理想合成图片上验证算法。",
    )
    ...
    add_para(
        doc,
        "这里需要特别说明 Normalize 与 epsilon 的关系。攻击中的 epsilon 设置在归一化后的 tensor 空间，而不是直接在 0 到 255 的像素值上设置。为了避免误解，后续扰动可感知性实验会把对抗样本反归一化回像素空间，再计算 Linf、平均绝对扰动和 PSNR。这样可以证明实际送入模型的对抗图像与原图非常接近，报告中的 Delta heatmap 只是为了展示扰动位置而做的放大视图。",
    )
...
def part4(doc, data):
    ...
    add_para(
        doc,
        "本部分按“先建立基线、再施加攻击、再验证扰动、再做对照和防御”的顺序展开。这样组织不是为了堆结果，而是为了让每个实验回答一个明确问题：基础识别回答模型是否可用；攻击实验回答模型是否脆弱；扰动可感知性回答攻击是否真实小扰动；随机噪声和 PGD 步数消融回答攻击机制是否成立；防御实验回答能否恢复；更严格验证回答防御是否存在边界。",
    )
    add_para(
        doc,
        "阅读本部分图表时应同时关注三类指标。第一类是准确率，例如 clean accuracy、adversarial accuracy 和 defended accuracy，用来衡量最终识别是否正确；第二类是攻击或恢复比例，例如 attack success rate 和 recovery rate，用来解释样本层面的变化；第三类是过程和可感知性指标，例如 confidence drop、PSNR、Top-1/Top-2 margin 和运行耗时，用来说明为什么结果会发生以及是否具备展示和部署意义。",
    )
    h2(doc, "4.1 基础识别结果")
    ...
    add_para(
        doc,
        "基础识别部分的结论是：模型已经具备较强的正常识别能力，但并非完美。97.81% 的 clean accuracy 说明后续攻击不是建立在一个弱模型上；混淆矩阵和错误样例则提醒我们，模型仍存在类别相似、图像质量差和局部细节不足导致的边界区域。后续对抗攻击正是在这些高维决策边界附近寻找最容易让模型失败的方向。",
    )

    h2(doc, "4.2 对抗攻击结果与指标分析")
    add_para(
        doc,
        "攻击实验的目的不是让图片看起来明显变坏，而是在严格扰动预算内最大化模型损失。为了形成可解释的对比，实验同时扫描多个 epsilon，并分别运行 FGSM 与 PGD。这样可以观察扰动预算增大时模型性能如何变化，也可以比较单步攻击和多步攻击在相同预算下的差异。",
    )
    ...
    add_para(
        doc,
        "从实验结果看，FGSM 和 PGD 都能显著降低准确率，但 PGD 在 epsilon=0.03 时更强。这说明 ResNet18 的 clean 决策边界在部分样本附近较近，且多步梯度搜索能够更稳定地找到跨越边界的扰动方向。这个结论为后续防御实验提供了目标：防御方法必须在不严重破坏正常图像的前提下，让样本重新回到模型较稳定的判别区域。",
    )
    ...
    add_para(
        doc,
        "这一节专门回应“攻击是否太明显”的问题。真实对抗图是 Original 与 Adversarial 两列，两者在视觉上非常接近；Delta x30 和 Delta heat 是人为放大或归一化后的分析视图，作用是让报告能看见扰动分布。数值上，epsilon=0.03 的像素空间 Linf 约为 0.008，约等于 2/255，PSNR 约 42 到 44 dB，属于小幅像素变化。因此本项目的攻击不是把图片明显改坏，而是标准意义上的小扰动对抗攻击。",
    )
    ...
    add_para(
        doc,
        "随机噪声对照是整个攻击证据链中非常关键的一环。如果同样 epsilon 下随机噪声也能大幅降低准确率，那么攻击结果可能只是图像质量下降造成的；但实验中随机扰动几乎保持 clean 准确率，而 FGSM/PGD 显著破坏识别。这说明有效性来自梯度方向的选择。换句话说，对抗扰动虽然像噪声，但它不是任意噪声，而是根据模型损失函数定向构造出来的高敏感方向。",
    )

    h2(doc, "4.5 消融实验二：PGD 迭代步数")
    add_para(
        doc,
        "PGD 步数消融进一步解释多步攻击为什么更强。这里固定 epsilon=0.03 和 alpha=0.005，只改变 steps，等于让攻击在同一个扰动球内搜索不同次数。如果步数增加后攻击更强，就能说明 PGD 的优势来自更充分的局部优化，而不是来自更大的扰动幅度。",
    )
    ...
    add_para(
        doc,
        "这个消融实验的意义在于排除“PGD 只是参数更多所以看起来强”的误解。实验中每一步都被投影回同一个 Linf epsilon ball，扰动预算没有变，改变的只是搜索是否充分。因此步数越多准确率越低，反映的是损失曲面中存在更多可被迭代发现的脆弱方向。",
    )
    ...
    add_para(
        doc,
        "Top-1/Top-2 margin 把攻击影响从“是否错分”进一步推进到“决策有多稳定”。即使某些样本在攻击后尚未错分，margin 下降也说明第一候选类别和第二候选类别的概率差距被压缩，模型已经更接近翻转边界。这个指标与 accuracy 互补，使报告不只停留在最终正确率，而能解释模型内部决策信心的变化。",
    )

    h2(doc, "4.7 输入防御结果与参数扫描")
    add_para(
        doc,
        "防御实验从一个现实问题出发：如果不重新训练模型，能不能通过简单输入变换减轻对抗扰动影响？选择 Gaussian Blur、Median Filter 和 JPEG Compression，是因为它们分别代表空间域平滑、空间域非线性局部统计和频域有损压缩。三者部署简单、解释直观，适合作为轻量防御基线；但它们也可能损伤交通标志细节，所以必须同时看防御后准确率和 clean accuracy 代价。",
    )
    ...
    add_para(
        doc,
        "因此，防御章节的判断标准不是“某个方法能否把所有样本恢复”，而是看它在同一威胁模型下能恢复多少、正常样本会损失多少、不同参数是否存在稳定趋势、面对更强攻击假设时是否仍然成立。后续参数扫描、JPEG quality 消融、失败案例和 BPDA 自适应攻击，就是围绕这四个问题展开的。",
    )
    ...
    add_para(
        doc,
        "JPEG 消融把防御分析从“哪种方法最好”推进到“为什么这个参数合适”。Q50 压缩更强，能破坏更多高频扰动，因此鲁棒准确率更高；但它也更容易损伤正常交通标志中的数字和边缘。Q90 更接近原图，clean accuracy 代价小，但对抗恢复弱。Q75 不是任意选择，而是在 clean 性能和 robust 性能之间较均衡的折中点。",
    )
    ...
    add_para(
        doc,
        "Grad-CAM 的作用是把指标变化转化为更容易展示的模型内部证据。攻击后模型不只是输出标签变了，它依赖的图像区域也可能偏离标志主体；防御后若预测恢复，关注区域也更接近原本的标志区域。虽然 Grad-CAM 不是严格因果证明，但它能帮助答辩时说明攻击和防御不是黑箱数字变化，而是伴随特征关注方式的变化。",
    )
    ...
    add_para(
        doc,
        "全测试集和运行效率验证让报告更接近工程项目，而不只是算法展示。完整 12630 张测试图验证说明 3000 张抽样实验不是偶然；运行耗时说明不同模块在实际演示中的取舍：clean 推理和 JPEG 防御可以即时展示，FGSM 可以快速生成，PGD 因多步反向传播更适合预生成样例或小批量演示。这一部分能支撑 Demo 设计，也体现出对技术成本的理解。",
    )

    h2(doc, "4.7.5 更严格防御验证：类别鲁棒性、失败案例、自适应攻击与对抗训练")
    add_para(
        doc,
        "前面的防御结果主要回答平均准确率是否提升。为了让证据链更严谨，本项目进一步补充四个验证：第一，按类别统计 PGD 脆弱性和 JPEG 恢复率；第二，展示 JPEG 仍无法恢复的失败样例；第三，使用 BPDA 思想评估攻击者知道 JPEG 防御时的自适应攻击；第四，进行 FGSM 对抗训练，验证模型级防御是否优于单纯输入预处理。",
    )

    per_class_df = data["per_class_robustness"]
    vulnerable = per_class_df[per_class_df.clean_correct >= 5].sort_values("attack_success_rate_on_clean_correct", ascending=False).head(8)
    rows = []
    ...
    table(
        doc,
        "PGD epsilon=0.03 下最脆弱类别示例",
        ["Class", "Label", "Images", "PGD Acc.", "JPEG Acc.", "Attack Success", "Recovery"],
        rows,
        [650, 2450, 850, 1200, 1200, 1550, 1460],
        "本表把整体平均结果拆到类别层面。Attack Success 是在 clean 正确样本中被 PGD 攻击成功的比例；Recovery 是这些成功攻击样本中被 JPEG Q75 恢复的比例。类别差异说明鲁棒性并不是均匀分布的，样本少、形状相近或细节弱的类别更容易成为薄弱点。",
        font_size=7.3,
    )
    figure(
        doc,
        "fig_42_per_class_attack_success.png",
        "PGD 下攻击成功率最高的类别",
        "该图按攻击成功率排序展示最脆弱类别。End speed and passing limits、End no passing、End speed limit 80 等类别更容易被攻击，说明类别相似性和样本规模会影响模型局部决策边界。",
        width=6.05,
    )
    figure(
        doc,
        "fig_43_per_class_defense_recovery.png",
        "JPEG Q75 恢复率最高的类别",
        "该图展示 JPEG 防御在不同类别上的恢复差异。某些类别恢复率很高，说明其扰动结构容易被压缩破坏；另一些类别恢复率低，说明输入预处理并非对所有类别都可靠。",
        width=6.05,
    )
    add_para(
        doc,
        "类别级分析把平均准确率拆开看。平均数能说明总体趋势，但会掩盖某些类别特别脆弱或特别容易恢复的现象。交通标志中有些类别形状非常相近，例如不同限速数字、解除限速和解除禁止超车等，模型决策边界可能更近；还有些类别样本较少，特征学习不够充分。按类别统计能让报告指出具体薄弱点，而不是只给出一个总准确率。",
    )
    ...
    table(
        ...
        rows,
        [650, 2400, 750, 750, 750, 1000, 1000, 2060],
        "本表专门展示失败案例。可以看到部分样本在 PGD 后以很高置信度错分，JPEG 后仍然维持错误预测。这说明输入防御只会破坏部分扰动，并不能保证所有样本恢复。",
        font_size=7.2,
    )
    figure(
        doc,
        "fig_44_pgd_jpeg_failure_examples.png",
        "PGD 攻击后 JPEG Q75 仍失败的样例",
        "该图从左到右展示 Original、PGD、JPEG Q75。它补充了报告的失败案例分析：防御有效不等于总能恢复，尤其在类别相似、标志细节弱或攻击后置信度很高时，压缩仍可能无法把样本拉回正确类别。",
        width=6.20,
    )
    add_para(
        doc,
        "失败案例是防御结论中最重要的约束条件。JPEG 可以恢复一部分对抗样本，但当 PGD 已经把样本推到错误类别深处，或者交通标志本身细节较弱、类别相似度较高时，压缩并不一定能把样本拉回正确区域。把失败案例放进报告，可以避免把输入预处理夸大成“安全方案”，也能为后续引入模型级防御提供动机。",
    )
    ...
    rows = [
        ...
        ("Standard PGD + JPEG Q75", pct(adaptive.standard_pgd_accuracy_after_jpeg), "先攻击再压缩，非自适应威胁模型"),
        ("Adaptive BPDA + JPEG Q75", pct(adaptive.adaptive_bpda_accuracy_after_jpeg), "攻击时把 JPEG 近似纳入优化"),
    ]
    table(
        doc,
        "JPEG 防御下普通 PGD 与自适应 BPDA 攻击对比",
        ["场景", "Accuracy", "含义"],
        rows,
        [3000, 1600, 4760],
        "BPDA 是 Backward Pass Differentiable Approximation，即前向传播保留不可导或难导的 JPEG 变换，反向传播用可导近似传梯度。该表用于检验输入预处理防御在攻击者知道防御存在时是否仍可靠。",
        font_size=8.0,
    )
    figure(
        doc,
        "fig_45_adaptive_jpeg_bpda_accuracy.png",
        "JPEG 防御在普通 PGD 与自适应攻击下的准确率",
        f"图中可见普通 PGD+JPEG Q75 准确率为 {pct(adaptive.standard_pgd_accuracy_after_jpeg)}，自适应 BPDA+JPEG Q75 为 {pct(adaptive.adaptive_bpda_accuracy_after_jpeg)}。下降幅度不大，但说明防御结论依赖攻击假设，不能把 JPEG 解释成完备安全方案。",
        width=5.95,
    )
    figure(
        doc,
        "fig_46_adaptive_jpeg_bpda_examples.png",
        "自适应 BPDA 攻击使 JPEG 防御失效的样例",
        "该图展示 clean、普通 PGD+JPEG 和 BPDA+JPEG 的对比。普通攻击下 JPEG 仍可恢复的部分样本，在自适应攻击下会再次错分，说明攻击者若知道防御流程，可以针对防御构造更有效扰动。",
        width=6.20,
    )
    add_para(
        doc,
        "BPDA 自适应攻击的意义在于改变威胁模型。普通 PGD 默认攻击者不知道或不优化 JPEG 防御，因此“先攻击再压缩”会让 JPEG 看起来更有效；BPDA 则假设攻击者知道防御流程，并在反向传播中用近似梯度绕过不可导的 JPEG。实验中 BPDA 后准确率进一步下降，说明输入预处理防御存在边界。即使下降幅度不大，这个实验也能证明报告没有停留在乐观防御评估，而是主动检验了更严格攻击假设。",
    )
    ...
    rows = [["Clean", "-", pct(base_clean), pct(clean_row.accuracy), pct(clean_row.accuracy - base_clean)]]
    ...
    table(
        ...
        rows,
        [1100, 800, 2200, 2300, 2960],
        "本表比较同一批 3000 张测试样本上的 baseline ResNet18 与 FGSM 对抗训练模型。对抗训练从已有 best checkpoint 继续微调 4 个 epoch。结果显示 clean 准确率未明显下降，而 FGSM/PGD 鲁棒准确率均显著提升。",
        font_size=8.0,
    )
    figure(
        doc,
        "fig_47_adv_training_loss_curve.png",
        "对抗训练损失曲线",
        "该图展示微调过程中 adversarial training loss 的下降。损失下降说明模型逐步适应了由 FGSM 生成的局部扰动样本。",
        width=5.80,
    )
    figure(
        doc,
        "fig_48_adv_training_accuracy_curve.png",
        "对抗训练准确率曲线",
        "该图同时展示训练对抗准确率和验证 clean 准确率。验证 clean accuracy 保持高位，说明短轮微调没有明显破坏正常识别能力。",
        width=5.80,
    )
    figure(
        doc,
        "fig_49_adv_training_robust_curve.png",
        "对抗训练前后鲁棒准确率曲线",
        "该图把 baseline 与 adversarial training 放在同一坐标系比较。对抗训练后 FGSM 和 PGD 曲线整体上移，说明模型级防御比单纯输入变换更直接地改善决策边界。",
        width=5.95,
    )
    figure(
        doc,
        "fig_50_adv_training_eps003_bar.png",
        "epsilon=0.03 下 baseline 与对抗训练模型对比",
        "该柱状图适合答辩时快速展示模型级防御收益。PGD epsilon=0.03 从 baseline 的 60% 左右提升到 88.63%，说明训练阶段加入对抗样本能显著提升鲁棒性。",
        width=5.95,
    )
    figure(
        doc,
        "fig_51_adv_training_robust_examples.png",
        "对抗训练模型抵抗 PGD 的样例",
        "该图展示对抗训练模型在 PGD epsilon=0.03 下仍能正确识别的样例。它补充了数值指标之外的可视化证据，说明防御不是只发生在平均统计上。",
        width=6.05,
    )
    add_para(
        doc,
        "对抗训练把防御从输入层推进到模型参数层。输入预处理是在测试时尝试清洗样本，而对抗训练是在训练时让模型反复见到扰动样本，从而学习更平滑的局部决策边界。本实验使用 FGSM 微调 4 个 epoch 后，clean accuracy 没有明显下降，同时 FGSM 和 PGD 鲁棒准确率显著提升。这说明真正强的防御通常需要改变模型本身，而不仅是对输入做后处理。",
    )

    h2(doc, "4.8 深度反思：成功、失败与原因")
    add_para(
        doc,
        "综合前面的实验，可以把本项目的现象分成三层理解。第一层是识别层：ResNet18 能在正常交通标志上取得高准确率，但仍会受图像质量和类别相似性影响。第二层是攻击层：小幅、定向、可微优化得到的扰动能显著改变模型预测，随机噪声对照说明关键不是扰动幅度本身，而是扰动方向。第三层是防御层：输入预处理能恢复一部分样本，但面对失败案例和自适应攻击仍有上限；对抗训练则通过改变决策边界提供更强的鲁棒性证据。",
    )
    rows = [
        ...
        ("防御失败", "强攻击或过强压缩导致细节丢失", "轻量输入防御有上限，失败案例和 BPDA 验证体现了边界"),
        ("对抗训练成功", "模型级防御显著提升 FGSM/PGD 准确率", "训练时直接优化局部扰动邻域内的分类稳定性"),
    ]
    table(
        doc,
        "典型现象与原因分析",
...
def part5(doc):
    ...
    add_para(
        doc,
        "Demo 的叙事重点应当是“同一张图像如何在模型中经历不同状态”。可以先让观众看到 clean 图像被正确识别，再展示加入肉眼不明显的小扰动后模型预测翻转，然后展示 Delta x30 或 heatmap 解释扰动位置，接着用随机噪声对照说明不是普通噪声，最后展示 JPEG 防御和对抗训练带来的恢复效果。这样比单纯播放曲线更有展示性，也能把原理、实验和结果串成一个可讲述的过程。",
    )
    rows = [
        ...
        ("步骤 7", "讲解防御边界", "展示 fig_44/fig_45，说明失败案例和自适应攻击"),
        ("步骤 8", "展示模型级防御", "展示 fig_50，说明对抗训练带来的鲁棒性提升"),
    ]
    table(
        doc,
        "Demo 演示脚本",
...
def part6(doc, data):
    ...
    adaptive = data["adaptive_jpeg"].iloc[0]
    adv_train_pgd003 = data["adv_training_robust"][
        (data["adv_training_robust"].attack == "pgd")
        & (np.isclose(data["adv_training_robust"].epsilon, 0.03))
    ].iloc[0]
    rows = [
        ...
        ("自适应攻击", "BPDA + JPEG Q75 accuracy", pct(adaptive.adaptive_bpda_accuracy_after_jpeg), "输入预处理防御存在威胁模型边界"),
        ("模型级防御", "Adv training PGD eps=0.03 accuracy", pct(adv_train_pgd003.accuracy), "对抗训练显著提升鲁棒性"),
    ]
    table(
        doc,
        "项目核心结论",
    ...
    add_callout(
        doc,
        "项目心得",
        "本项目最大的收获是：深度学习模型的高准确率不等于高安全性。对抗攻击展示了模型决策边界的脆弱性，随机噪声对照证明了梯度方向的重要性，防御参数扫描和 JPEG quality 消融说明鲁棒性提升往往伴随正常性能损失。新增的失败案例和 BPDA 自适应攻击说明输入预处理不是完备安全方案；FGSM 对抗训练则证明从模型参数层面提升鲁棒性更有效。Grad-CAM 从关注区域角度补充了模型内部证据。后续如果继续推进，应重点补充 PGD/TRADES 训练、跨模型消融和交互式 Demo。",
    )
    h2(doc, "6.1 实验链路总结")
    add_para(
        doc,
        "本项目最终形成了比较完整的实验闭环。首先，ResNet18 在 GTSRB 上取得较高 clean accuracy，说明基础识别任务成立；其次，FGSM 和 PGD 在 Linf 约束下显著降低准确率，说明模型存在对抗脆弱性；再次，扰动可感知性分析把归一化 epsilon 转换到像素空间，证明攻击不是明显改图；随后，随机噪声对照、PGD 步数消融和概率间隔分析进一步解释攻击为什么有效；最后，输入防御、参数扫描、JPEG 消融、失败案例、BPDA 和对抗训练共同构成防御评估链路。",
    )
    add_para(
        doc,
        "这条链路的价值在于每一步都回答前一步留下的问题。clean 准确率高之后，问题变成模型是否安全；攻击成功之后，问题变成扰动是否真实微小；确认扰动微小之后，问题变成它是否只是随机噪声；证明是梯度方向之后，问题变成能否防御；防御有效之后，问题又变成防御是否稳健、是否有失败边界、是否能被自适应攻击绕过。报告按这个逻辑展开，比单独罗列图表更能体现实验设计能力。",
    )
    h2(doc, "6.2 方法局限与改进方向")
    add_para(
        doc,
        "当前项目仍有局限。第一，基础模型只深入验证了 ResNet18，虽然它适合作为课程项目主干，但还没有与 SimpleCNN、原始 ImageNet stem ResNet18 或更轻量模型做完整结构消融。第二，对抗训练采用 FGSM 微调，已经证明模型级防御有效，但还不能替代更严格的 PGD adversarial training 或 TRADES。第三，BPDA 自适应攻击只验证了 JPEG Q75 的一种近似攻击，还可以继续加入 EOT、多随机重启和更多攻击步数。第四，Demo 目前以离线素材为主，如果时间允许，可以进一步做成上传图片即时识别、攻击和防御的交互界面。",
    )
    add_para(
        doc,
        "后续最值得补强的方向是 PGD/TRADES 对抗训练和跨模型复验。PGD/TRADES 能更系统地优化鲁棒性，避免 FGSM 训练可能出现的梯度遮蔽或过拟合；跨模型复验则能证明当前结论不是 ResNet18 特有。如果再配合交互式 Demo，项目就能同时具备理论解释、实验严谨性和展示效果。",
    )
    h2(doc, "6.3 个人/小组心得")
    add_para(
        doc,
        "从实践角度看，本项目让我们体会到机器学习实验不能只追求一个好看的准确率。最初 clean accuracy 达到较高水平时，模型看起来已经“很好”；但攻击实验说明，高准确率背后仍可能存在非常近的决策边界。真正有说服力的报告需要保存中间过程、保留失败案例、做对照实验，并把每个指标解释清楚。特别是扰动热力图容易被误解为真实攻击图，因此必须用 PSNR、Linf 和放大倍数说明展示图与实际输入之间的区别。",
    )
    add_para(
        doc,
        "从技术理解看，最重要的收获是把深度学习、对抗攻击和防御放到同一套数学框架中理解：分类训练是在最小化损失，对抗攻击是在输入邻域内最大化损失，输入防御是在测试时改变输入分布，对抗训练是在训练时最小化邻域内的最坏损失。这样理解后，FGSM、PGD、JPEG、防御参数扫描和对抗训练不再是孤立脚本，而是围绕决策边界稳定性展开的连续实验。",
    )
    add_para(
        doc,
        "从报告展示角度看，最有效的表达方式不是把所有图堆在一起，而是让图表服务于问题链：为什么需要这个实验、这个图说明了什么、它排除了哪个疑问、它还不能证明什么。按照这个思路，本报告把平均指标、类别分析、可视化样例、失败案例和运行效率都保存下来，既能支撑答辩讲解，也能让老师看到实验不是简单复现，而是有完整证据链和反思的鲁棒性研究项目。",
    )
...
def build_docx():
    ...
    try:
        ...
        fallback = OUT_PATH.with_name(OUT_PATH.stem + "-text-enhanced" + OUT_PATH.suffix)
        doc.save(fallback)
        return fallback
